...
def cache_context(
    ...
    tags: list[str],
    category: str = "",
    related_classes: dict | None = None,
    summary: str = "",
    project_root: str = ".",
) -> str:
    """
    소스 코드 탐색 결과를 로컬 캐시에 저장한다.
    서버 미응답 시 폴백 검색에서 활용된다.
    RAG는 네비게이션 용도이므로, 실제 판단은 반드시 소스 코드를 직접 읽어야 한다.

    Args:
        file_path: 소스 파일의 상대 경로 (예: "Combat/DamageSystem.h")
        tags: 관련 태그 목록 (예: ["전투", "데미지", "HP"])
        category: 카테고리 경로 (예: "전투/데미지/시스템")
        related_classes: 관련 클래스 딕셔너리 (예: {"UDamageSystem": "Combat/DamageSystem.h"})
        summary: 파일 기능 한줄 요약 (선택)
        project_root: 프로젝트 루트 경로 (기본값: 현재 디렉토리)
    """
    context_dir = Path(project_root) / ".claude" / "context"
    md_rel = str(Path(file_path).with_suffix('.md'))
    md_path = context_dir / md_rel
    md_path.parent.mkdir(parents=True, exist_ok=True)

    rc = related_classes or {}
    ...
    if rc:
        lines.append("related_classes:")
        for cls_name, cls_path in rc.items():
            lines.append(f"  - {cls_name}: {cls_path}")
    lines.append("---")
    lines.append("")
    if summary:
    ...
    md_path.write_text("\n".join(lines), encoding="utf-8")
    _cs_log(f"캐시 저장 (cache_context): {md_rel}")
